...
| 3 | Dataset-tab edits don't change training data | **P0 — correctness** | Same param-map gap + no dataset-swap endpoint                    | canopy adapter + cascor + data-client           |
...
| 6 | Left sidebar too wide on Training Metrics    | P3 — cosmetic        | Hardcoded `dbc.Col(width=3)` for all tabs                        | per-tab width via existing `TAB_SIDEBAR_CONFIG` |

**Recommended ordering** (justified in §10):

```bash
5 → 1 → 3 → 4 (skeleton) → 2 → 6 → 4 (full coverage)
```
...
* `tests/regression/test_no_silent_dataset_drop.py` — for each id whose callback writes `nn_dataset_*` / `nn_spiral_*`, assert the *destination* is either `_DATASET_PARAMS` or explicitly listed in a canopy-only allowlist.
...
+      - run: pytest -m ui src/tests/ui --maxfail=3 -q
```
...
* `TAB_SIDEBAR_CONFIG` is already the per-tab knob registry — adding a `sidebar_width_class` field is a clean extension.
* Keeps all global layout invariants (grid still sums to 12 with `width="auto"` + flex-grow on the right column).
* C is appealing but is a separate UX research question.

Recommended widths (Bootstrap classes):

| Tab                                                           | Sidebar width       | Right width |
|---------------------------------------------------------------|---------------------|-------------|
| metrics, candidates, topology, dataset, network-editor        | `width=3` (current) | `width=9`   |
| boundaries, evolution, parameters, snapshots, replay, workers | `width=2`           | `width=10`  |
| about, tutorial, redis, cassandra                             | `width=2`           | `width=10`  |

(I.e. all *non-Training-Metrics tabs that don't show Network Parameters* shrink to width=2 — frees ~8% of the viewport for the visualisation.  The user's specific request to *shrink Training Metrics* itself is more risky; do a Playwright check from §4 first.)
...
def _resize_sidebar(active):
    cfg = TAB_SIDEBAR_CONFIG.get(active, {})
    sidebar = cfg.get("sidebar_width", 3)
    return sidebar, 12 - sidebar
```

### 6.6 Tests for the fix

* `tests/regression/test_tab_sidebar_widths.py` — for every entry in `TAB_SIDEBAR_CONFIG`, assert `sidebar_width + visualization_width == 12`.
* `tests/ui/test_sidebar_width.py` — switch tabs in Playwright, assert the sidebar `<div>`'s computed `clientWidth` differs between Training Metrics and About by ≥ 50px.
...
Five PATCH targets (§1) + one POST endpoint (§3).
...
`src/demo_mode.py` mirrors the cascor surface. Every new param / route needs a demo equivalent or the tests under `JUNIPER_CANOPY_DEMO_MODE=1` will skip silently.
...
PR-4: cascor — five new PATCH endpoints + tests                 [issue 1, server side]
PR-5: canopy — extend _CANOPY_TO_CASCOR_PARAM_MAP + roundtrip   [issue 1, finished]
PR-6: cascor — stage_dataset endpoint + reload                  [issue 3, server side]
PR-7: canopy — Apply Dataset UI + adapter wiring                [issue 3, finished]
PR-8: canopy — debounce=350ms sweep + clientside blur + invalid [issue 2]
PR-9: canopy — per-tab sidebar width                            [issue 6]
...
| PR-4      | 4h         | 8h                               |
| PR-5      | 2h         | 4h                               |
| PR-6      | 4h         | 12h (juniper-data integration)   |
| PR-7      | 4h         | 8h                               |
...
graph TD
  ...
  PR3 --> PR4[PR-4 cascor PATCH endpoints]
  PR4 --> PR5[PR-5 extend canopy map]
  PR5 --> PR8[PR-8 debounce sweep]
  PR3 --> PR6[PR-6 cascor stage_dataset]
  PR6 --> PR7[PR-7 canopy Apply Dataset]
  PR7 --> PR9[PR-9 sidebar width]
...
* **PR-6 → PR-7**: same logic for `stage_dataset`.
...
* `pip install playwright` + `playwright install chromium` adds ~250MB to CI cache. Coordinate with `.github/workflows/ci.yml` cache budget. May require a separate caching strategy (action `actions/cache` keyed on `playwright --version`).
* Demo mode (`src/demo_mode.py`) needs to mirror new endpoints. If demo mode is being deprecated (see prior `notes/`), confirm before duplicating.
...
4. **UX cleanup last**: PR-8 (debounce), PR-9 (sidebar) require no backend coordination and are safe to land last.
5. **Coverage-completion last**: PR-10 expands the UI suite once all fixes are in, so baselines are stable.
...
* Issue #6's recommended *narrowing* is for non-Training-Metrics tabs only (the inverse of the user's specific ask).
  * The Training-Metrics-tab side carries the longest labels and inputs.
  * If the user does want it narrower there too, recommend a Playwright width-experiment first to find the break-point empirically before committing to a number.
...
1. Demo mode: is it being deprecated soon? If so, PR-6/7 don't need a demo mirror.
    * **Answer 1**: short answer, no.  there are no plans that currently exist for deprecatign demo mode.  however, i am open to discussion about possible approaches to leverage actual cascor & canopy functionality (minimizing the need to resort to a second, demo-specific instance of objects and codeblocks)

2. Are the candidate-pool selection knobs (`cn_selected_candidates`, `cn_top_candidates`, `cn_random_candidates`) still part of the intended product, or vestigial UI? Confirms whether PR-4 is worth doing.
    * **Answer 2**: yes, all three are part of the intended project.  they are intended to capture the following:
      * `cn_selected_candidates`: stores the value for the number of candidate nodes selected after each candidate pool finishes cascor training.  this is part of the plumbing needed to allow multiple candidate nodes to be added at one time in network-layer-like manner.
      * `cn_top_candidates`: store the number of the top N candidates to include in the selected candidates parameters. if the random candidates parameter is set to 0, not used, the top candidates param should be the same value as the selected candidates param.
      * `cn_random_candidates`: store the number of randomly selected candidate pool nodes to be included in the selected candidates parameter. if the top candidates param is 0, the random candidates param should equal the selected candidates param.  if both top candidates and random candidates have non-zero values, their sum should equal the selected candidates param.

3. What's the CI minutes budget? PR-3 + PR-10 will add ~3-5 min/run.
    * **Answer 3**: let's try to keep the CI run time from increasing by more than ~5min.  ultimately, however, accuracy is more important than speed for CI.

4. Is there a brand-spec on the sidebar width or is this purely engineering judgement? Determines how aggressively to narrow.
    * **Answer 4**: no brand-spec exists currently.  beginning the process of standardizing the UI specs is something that should come out of this work and should involve the UI test suite as well.
