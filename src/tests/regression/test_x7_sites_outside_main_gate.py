...
class TestSitesOutsideMainGateStayOffLoop:
    ...
    async def test_relay_cascade_add_does_not_block_the_loop(self):
        ...
        assert ticker.worst_gap < STALL_SECONDS, (
            f"the relay stalled the loop for {ticker.worst_gap:.3f}s -- extract_network_topology is back on the loop"
        )
    ...
    async def test_the_ticker_detects_an_inline_block(self):
        ...
        assert ticker.worst_gap >= BLOCK_SECONDS * 0.8, (
            f"the ticker's worst gap was {ticker.worst_gap:.3f}s against an inline "
            f"{BLOCK_SECONDS}s sleep -- this harness cannot measure X7"
        )
