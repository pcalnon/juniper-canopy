...
def test_exemption_is_site_local_not_expression_global():
    ...
    assert "health_check" in flagged, "inline backend.get_status() was not flagged -- exemption is expression-global " "again, and a single offload will hide the health endpoints"
...
def test_offloaded_closure_is_not_flagged():
    ...
    assert not _blocking(buckets), f"the correct to_thread(_fetch) idiom was flagged: {_blocking(buckets)}"
...
def test_redis_client_is_other_and_blocking():
    ...
    assert any("handler" in hit for hit in buckets["OTHER"]), f"get_redis_client() was not classified OTHER: {buckets}"
...
def test_unbound_io_receiver_is_unresolved_and_blocking():
    ...
    assert any("handler" in hit for hit in buckets["UNRESOLVED"]), f"unbound client.get_status() was not UNRESOLVED: {buckets}"
    assert any("handler" in hit for hit in _blocking(buckets))
...
def test_live_io_is_not_on_the_verified_no_io_list():
    ...
    assert not overlap, f"{sorted(overlap)} added to VERIFIED_NO_IO_CALLS -- that hides every " "matching call, including the health endpoints"
...
def test_gate_blocking_composition_includes_other_and_unresolved():
    ...
    assert names == {"CASCOR", "OTHER", "HELPER", "UNRESOLVED"}, f"gate blocking composition drifted: {sorted(names)}"
