...
class TestPollerBudget:
    def test_worst_case_concurrent_pollers_within_budget(self, dashboard):
        ...
        assert total <= POLLER_BUDGET, (
            f"worst-case {total} concurrent perpetual pollers "
            f"({global_n} global + {worst_n} on the '{worst_tab}' tab) "
            f"exceeds the budget of {POLLER_BUDGET} (renderer cap {RENDERER_SLOT_CAP}). "
            f"Gate the new poller to its tab instead of raising the budget."
        )
