"""
Pin the CPU-only torch contract between the Dockerfile, the publish workflow and the in-image
check script.

Regression guard for the class the worker's first published image fell into (2026-09-07:
``torch 2.12.1+cu130`` plus the ``nvidia-*`` / ``triton`` stack, 3 GB per node, because the
Dockerfile installed torch unpinned from the CPU index and then let a lock install re-resolve it
from PyPI). canopy's ``requirements.lock`` carries no torch -- torch is a ``[demo]`` extra -- so
the Dockerfile installs it separately for demo mode, and that install must be pinned to a
``+cpu`` build in BOTH pip invocations with the CPU index visible to the lock install.

Each test below fails on one link of that chain drifting. None of them build an image; the
...
class TestDockerfileTorchPin:
    def test_lock_carries_neither_torch_nor_the_cuda_stack(self):
        """torch is a [demo] extra; if it ever lands in the lock it will be PyPI's CUDA build."""
        offenders = [p for p in _pins(LOCK) if p.startswith("torch==") or p.startswith(CUDA_STACK_PREFIXES)]
        assert offenders == [], f"requirements.lock must not pin torch or any CUDA-stack package: {offenders}"

    def test_torch_install_is_pinned_cpu_wheel_from_cpu_index(self):
        version = _dockerfile_torch_version()
        installs = [r for r in _run_instructions() if "pip install" in r and "-r requirements.lock" not in r and "torch==" in r]
    ...
    def test_lock_install_carries_cpu_index_and_pin(self):
        """The link that broke the worker: the lock install must be able to SEE the CPU wheel and must be held to it."""
        version = _dockerfile_torch_version()
        ...
        assert f"torch=={version}+cpu" in line, line
        # --index-url would REPLACE PyPI, and the CPU index 403s most of the lock.
...
class TestPublishWorkflowAssertsContract:
    def test_paths_filter_covers_every_input_the_dockerfile_copies(self):
        """A filter that misses an input is a build that never runs for the change that mattered."""
        paths = _workflow()["on"]["pull_request"]["paths"]
        for required in ("Dockerfile", "requirements.lock", "pyproject.toml", "src/**", "juniper_canopy/**", "conf/app_config.yaml", "conf/logging_config.yaml", "conf/layouts/**", CHECK_SCRIPT_REL, ".github/workflows/publish-image.yml"):
    ...
    def test_smoke_test_runs_the_check_and_imports_the_package(self):
        run = _step("build", "Smoke test")["run"]
        assert CHECK_SCRIPT_REL in run
        assert "EXPECT_TORCH=" in run
        assert "import juniper_canopy" in run
        assert "--entrypoint" not in run, "this image is CMD-only; --entrypoint would mask a wrong entrypoint contract"
...
class TestCheckImageCpuOnly:
    @pytest.mark.parametrize("value", ["", "2.14.0", "2.14.0+cu130", "latest", "+cpu"])
