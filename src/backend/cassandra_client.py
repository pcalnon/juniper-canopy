...
class CassandraClient:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        """
        now = datetime.now()
        cache_valid = self._cached_status is not None and self._last_status_check is not None and (now - self._last_status_check).total_seconds() < self._status_cache_ttl_seconds
        if cache_valid:
