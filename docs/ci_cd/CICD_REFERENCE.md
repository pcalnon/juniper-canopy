...
### Lockfile Automation Specification

The lockfile workflow exists to keep `requirements.lock` aligned with `pyproject.toml` extras used by CI and Docker builds.
...
**Generation command used by automation:**
...
- Creates commit `[dependabot skip] Update requirements.lock` only when the file changed.
- Pushes using `CROSS_REPO_DISPATCH_TOKEN` checkout credentials so downstream CI re-runs.

#### Lockfile Freshness Check (CI)

The main CI pipeline enforces lockfile freshness by regenerating the lockfile with the same extras and diffing against the repository lockfile body.
...
- CI strips the first two lines of each file before diffing to ignore uv's autogenerated header comment containing the output path (`-o ...`), which differs between local and CI generation.
