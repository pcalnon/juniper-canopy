...
| Depth-filter label reads `"0 of N"` at rest, or ignores the slider | F-CANOPY-042, fixed in canopy#570: the label *was* a State of the bounds-sync callback and `0` ≠ `"all"` in the old JS rule | Do not add `Input(-depth-slider, value)` to that callback (circular); #570 gave the label its own callback instead. See [Hierarchy Depth Filter](AGENTS_REFERENCE.md#hierarchy-depth-filter-can-020) |
