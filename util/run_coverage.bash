...
#    the aggregate gate before pushing. Runs the FULL gated suite by design
#    (narrowing the selection would lower coverage); use plain pytest for a subset.
#
# Usage:
...
# ── Reproduce the CI coverage sequence (keep in sync with .github/workflows/ci.yml unit-tests job) ──
python -m pytest -m "not requires_cascor and not requires_server and not slow" src/tests/unit/ src/tests/regression/ --cov=src --cov-report=term-missing --cov-fail-under="${COVERAGE_FAIL_UNDER}"
# ────────────────────────────────────────────────────────────────────────────────────────────────────
