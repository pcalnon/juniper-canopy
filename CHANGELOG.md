...
- **#2b PARAM-SURFACE CLEANUP (structural follow-up to the honesty fix)**: completes the apply-params surface work. **(1) Data-loss fix:** `nn_output_epochs` / `nn_optimizer_type` / `nn_activation_function_name` were sent by the dashboard but **silently dropped** by `SetParamsRequest` (and the route's `nn_keys` allowlist) before reaching the adapter — which *does* map them (`output_epochs` / `optimizer_type` / `activation_function_name`). Added all three to `SetParamsRequest` (`src/main.py`) and to the route's `nn_keys`, so they now reach cascor. **(2) Relocation:** `nn_dataset_elements` / `nn_dataset_noise` are dropped from the `/api/set_params` payload — they're canopy-local and travel on `/api/stage_dataset` (Issue #4 cold-swap), the single intended channel. **(3) Read-only:** `cn_training_complete` (a status flag, not an editable param) is dropped from the payload. (The display/applied-params store is unchanged.) Tests: `test_apply_button_parameters.py::test_apply_handler_uses_correct_keys` (the 3 mappable params present in the POST body; the 3 relocated/read-only absent) + `test_meta_parameters_api.py::test_set_params_accepts_previously_dropped_params` (a payload of only the 3 is now recognized → 200, previously rejected as "no parameters"). Companion to the #2b honesty core (below) and the #337 dataset cold-swap.
