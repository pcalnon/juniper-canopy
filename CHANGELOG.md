...
- **Complementary X7 slice 1c guard tests** (`src/tests/regression/test_x7_status_cache_guards.py`),
  16 tests over the leftover paths `#578`'s T-C1–T-C4 suite cannot reach: a raising `normalize`
  ...
  `_start_status_cache` skips demo and adapterless service backends; the status-class gauge is
  one-hot on transition. Harvested from canopy#580, whose other nine files had already landed.
